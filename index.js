  ...
  if (
    ...
    newState.guild.channels
      .create(newState.member.user.username, {
        ...
        type: 2,
        reason: "Temp channel Bot by Waleed",
      })
      .then(async (channeltemp) => {
        await newState.setChannel(channeltemp, "Temp channel Bot by Waleed");
  ...
  if (interaction.isSelectMenu()) {
    if (interaction.customId.startsWith("temp_limit")) {
      ...
      if (
        ...
        return await interaction.reply({
          content: "ليس لديك الإذن بالتحكم في القناة المؤقتة ❌",
          ephemeral: true,
        });
